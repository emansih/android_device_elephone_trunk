...
LINEEND=" \\"
COUNT=`wc -l proprietary-files-crackling.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' proprietary-files-crackling.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' proprietary-files-crackling.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
